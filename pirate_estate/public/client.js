...
var Botkit = {
  ...
  send: function (text, e) {
    ...
    console.log("user send: " + JSON.stringify(message));
  ...
  renderMessage: function (message) {
    ...
    if (!message.isTyping) {
      ...
      displayMessage(message);  // Output without typing dots (undefined) 
      // xhrCall();
      // TODO: Pass var message to myscript.js

    }

    // Custom
    // console.log("renderMessage: " + JSON.stringify(message.text));
...
function displayMessage(chatMessage) {
  console.log("message from function displayMessage(): " + JSON.stringify(chatMessage.text));
  var output = chatMessage;
  // goToMyScript();
  // export { output };


  console.log("output.text: " + JSON.stringify(output.text));
  if (output.text === "Do you need anything else?") {
    console.log("Equals!");
    // xhrCall();
    console.log("call xhrCall()");
    xhrCall();
    console.log("~~~~~~~~~~~~~~~~~~~ NIKOS CODE FROM client.js");
  }
...
function xhrCall() {
  ...
  xhr.onload = function () {
    ...
    data.history.forEach(function (post) {
      ...
      console.log("properties from: " + data.history[0][0][4].area);
      ...
      // Update filters from chatbox
      // parent.$("#minPrice").attr({
      //   // "max": data.history[0][0][11].highestPriceChatFilter.toLocaleString(),
      //   // "min": data.history[0][0][10].lowestPriceChatFilter.toLocaleString()
      // });
      // parent.$("#minPrice").val("123");
      parent.$("#minPrice").val(data.history[0][0][10].lowestPriceChatFilter);

      // parent.$("#maxPrice").attr({
      //   "max": data.history[0][0][11].highestPriceChatFilter.toLocaleString()
      // });
      parent.$("#maxPrice").val(data.history[0][0][11].highestPriceChatFilter);

      parent.$("#price-range").text(data.history[0][0][10].lowestPriceChatFilter.toLocaleString() + " - " + data.history[0][0][11].highestPriceChatFilter.toLocaleString());

      // parent.$("#roomSlider").attr({
      //   "value": data.history[0][0][9].roomChatFilter
      // });
      parent.$("#roomSlider").val(data.history[0][0][9].roomChatFilter);

      parent.$("#rooms").attr({
        "value": data.history[0][0][9].roomChatFilter
      });

      // Change filter attributes depending on the flow
      if (data.history[0][0][8].flowFlag == "rent") {
        parent.$("#input-switch").addClass("isNotCheck");
        parent.$("#input-switch").removeClass("isCheck");
        ...
        parent.$("#input-switch").prop('checked', false);

        parent.$('#buy-label').css({ 'font-weight': '400', "color": "#adadad" });
        parent.$('#rent-label').css({ 'font-weight': '600', "color": "#fff" });
        
      } else if (data.history[0][0][8].flowFlag == "buy") {
        parent.$("#input-switch").addClass("isCheck");
        parent.$("#input-switch").removeClass("isNotCheck");

        if (data.history[0][0][11].highestPriceChatFilter > 500000) {
          parent.$("#minPrice").attr({
            "max": data.history[0][0][11].highestPriceChatFilter
          });
          parent.$("#maxPrice").attr({
            "max": data.history[0][0][11].highestPriceChatFilter
          });
      ...
      // Uppercase first letter of area for dropdown update 
      data.history[0][0][4].area = data.history[0][0][4].area.toLowerCase().replace(/\b[a-z]/g, function (letter) {
        return letter.toUpperCase();
      });
      parent.$("#locality-dropdown").val(data.history[0][0][4].area);

    });

    console.log("totalData length: " + totalData);
    parent.$("#main-conteiner").empty();
    parent.$("#icon_boxes").empty();
    for (var i = 0; i <= totalData; i++) {
      console.log("length: " + data.history.length);
      for (var y = 0; y <= totalData; y++) {
        // elm.appendTo(parent.$("#response"));
        var variableYes = "yes";
        parent.$("#main-conteiner").append(
          "<li class='cd-item " + data.history[0][0][8].flowFlag + "' id='" + data.history[0][y][i + 6].propertiesID + "'>"
          + "<img src=http://localhost/form/upload/" + data.history[0][y][i + 7].imageThumb + " alt='Item Preview'>"
          + "<a href='#0' class='cd-trigger'><span>Quick View</span></a>"
          + "<div class='property-info'><span class='property-info-price'>" + data.history[0][y][i + 2].price.toLocaleString() + "</span><span class='property-info-price-sign'>€</span><span class='property-info-size'>" + data.history[0][y][i + 3].size + "m<sup>2</sup></span></div>"
          + "<div class='property-info-area'><i class='fas fa-map-marker-alt'></i>" + data.history[0][y][i + 0].street + " " + data.history[0][y][i + 1].streetNumber + ", " + data.history[0][y][i + 4].area + "</div>"
          + "</li>"
        );
      }
    }

    // alert(data.history[0][0][4].area);



    parent.$("#slider-property-info").append(
      "<h2>" + data.history[0][y][i + 4].area + "</h2>"
    );
