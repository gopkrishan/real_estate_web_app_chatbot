module.exports = function (controller) {
   ...
   */
  var myVar = "hello I am var";

  var sourceFromBuyer = require('./buy_flow.js');
  var sourceFromTenant = require('./rent_flow.js');
  var toJson;
  // TODO:  Convert sourceFromBuyer.firstItem to JSON -> DONE: now it is send from buyer_flow as JSON.stringify()

  // if(sourceFromBuyer.firstItem != null){
  //   toJson = JSON.parse(sourceFromBuyer.firstItem);
  // }
  console.log("sourceFromBuyer.firstItem: " + sourceFromBuyer.firstItem);
  ...
  if (controller.storage && controller.storage.history) {
    ...
    controller.webserver.post('/botkit/history', function (req, res) {
      if (sourceFromBuyer.firstItem != null) {
        console.log(" ~~~~~~~~~~~~~ sourceFromBuyer.firstItem is not null")
        toJson = JSON.parse(sourceFromBuyer.firstItem);
      }else if (sourceFromTenant.firstItem != null) {
        console.log(" ------------- sourceFromTenant.firstItem is not null")
        toJson = JSON.parse(sourceFromTenant.firstItem);
      }
      res.json({ success: true, history: [toJson] });  // Send data to browser
      sourceFromBuyer.firstItem = null;  //FIXME: if not null another user sees the same data
      toJson = null;
    });
  }
