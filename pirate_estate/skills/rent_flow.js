...
module.exports = function (controller) {
    ...
    controller.hears('rent', 'message_received', function (bot, message) {
        var propertiesArray = [];
        var isAreaFlag;
        var minPriceDb;
        var maxPriceDb;
        ...
        bot.startConversation(message, function (err, convo) {
            ...
            convo.ask('So, give me the location you\'re interested in renting in Thessaloniki.', function (response, convo) {
                convo.setVar('area', response.text)
                var area = response.text;


                connection.query("SELECT MIN(PRICE) AS minAreaPrice FROM PROPERTIES JOIN RENT ON\
                ...
                connection.query("SELECT MAX(PRICE) AS maxAreaPrice FROM PROPERTIES JOIN RENT ON\
                PROPERTIES.ID = RENT.PROPERTY_ID WHERE AREA = ('" + area + " ')", function (error, results, fields) {
                                            ...
                                            if ((parseInt(highestPrice, 10) >= minPriceDb) && (parseInt(lowestPrice, 10) <= maxPriceDb) && (parseInt(highestPrice, 10) >= parseInt(lowestPrice, 10))) {
                                                ...
                                                convo.ask('Please set lowest number of rooms you want.', function (response, convo) {
                                                    ...
                                                    if (!isNaN(roomResponse)) {
                                                        ...
                                                        connection.query("SELECT PROPERTIES.ID, STREET, STREET_NUMBER, PRICE, SIZE, NUMBER_OF_ROOMS, AREA, PROPERTIES.IMAGE_NAME FROM PROPERTIES JOIN RENT ON\
                                                                  PROPERTIES.ID = RENT.PROPERTY_ID WHERE AREA = ('" + area + " ') AND\
                                                                ...
                                                                for (var i = 0; i < results.length; i++) {
                                                                    ...
                                                                    jsonVar[i] = [
                                                                        ...
                                                                        {"lowestPriceChatFilter": lowestPrice},
                                                                        {"highestPriceChatFilter": highestPrice}
                                                                    ];
